...
! !INTERFACE: ------------------------------------------------------------------
! !Notes:
! !06-10-2024: XJ modified it to make the calender follow exo_porb according to
!              Adams et al 2019
!             1yr = 1 exo_porb
!             1month = exo_porb // MONTHS_PER_YEAR + 1 from month 1 to
!             mod(int(exo_porb),MONTHS_PER_YEAR))
...
!EOP
   ...
   INTEGER :: I = 0
   integer(SHR_KIND_IN),parameter :: dpy = exo_porb
   integer(SHR_KIND_IN),parameter :: dpm(12) = &     ! Days Per Month
   &                    (/ (int(dpy)/12 + 1, I = 1, mod(int(dpy),12)), (int(dpy)/12, I = mod(int(dpy),12)+1,12) /)
