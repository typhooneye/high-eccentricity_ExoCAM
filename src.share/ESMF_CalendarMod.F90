! $Id$
! NOTES:
! 05-17-2024: XJ modified it to make the calender follow exo_porb according to Adams et al 2019
!             1yr = 1 exo_porb
!             1month = exo_porb // MONTHS_PER_YEAR + 1 from month 1 to mod(int(exo_porb),MONTHS_PER_YEAR))
!             and the remainder as exo_year_int % MONTHS_PER_YEAR
...
!------------------------------------------------------------------------------
      ...
      !ada: adapt it to exoplanets
      INTEGER, PARAMETER :: exo_year = exo_porb
      INTEGER :: I = 0
      INTEGER, PARAMETER :: mday(MONTHS_PER_YEAR) = &
      (/(int(exo_year)/MONTHS_PER_YEAR+1, I = 1, &
      mod(int(exo_year),MONTHS_PER_YEAR)), &
      (int(exo_year)/MONTHS_PER_YEAR, &
      I = mod(int(exo_year),MONTHS_PER_YEAR)+1,MONTHS_PER_YEAR) /)
      ! INTEGER, PARAMETER :: mday(MONTHS_PER_YEAR)   &
      !                     = (/31,28,31,30,31,30,31,31,30,31,30,31/)
      INTEGER, PARAMETER :: mdayleap(MONTHS_PER_YEAR) &
                          = (/31,29,31,30,31,30,31,31,30,31,30,31/)
      INTEGER, DIMENSION(int(exo_year)) :: daym
