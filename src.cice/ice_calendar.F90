...
! 2024 XJ: make the calendar can be applied to any orbital period
!          turn off the leap year option
!
! !INTERFACE:
!
      module ice_calendar
...
!
      ...
      integer (kind=int_kind) :: &
        ...
        ! integer(SHR_KIND_IN) :: daymo(12) = &     
        ! &                    (/ (int(days_per_year)/12 + 1, I = 1, mod(int(days_per_year),12)), (int(days_per_year)/12 + 1, I = mod(int(days_per_year),12)+1,12) /)
       !! day number at end of month
       !integer(SHR_KIND_IN) :: daycal(13)
       !integer(SHR_KIND_IN) :: i
...
!=======================================================================
      ...
      ! Input/output paramters
        ...
        integer, intent(out), optional :: daymo_out(12)     ! days permonth

      ! Local variables
        integer(kind=int_kind) :: daymo0(12)          ! number of daysin each month
        integer(kind=int_kind) :: daycal0(13)         ! day number atend of month
        integer(kind=int_kind) :: i_daycal, days_per_year

        ! Check if days_per_year_in is present, otherwise use a default
        ! value
        if (present(days_per_year_in)) then
          days_per_year = days_per_year_in
        else
          call abort_ice('ice: get_daycal needs days_per_year_in')
        end if
        
        do i_daycal = 1, mod(days_per_year, 12)
          daymo0(i_daycal) = days_per_year / 12 + 1
        end do

        do i_daycal = mod(days_per_year, 12) + 1, 12
          daymo0(i_daycal) = days_per_year / 12
        end do

        ! Initialize the first element of daycal0
        daycal0(1) = 0

        ! Calculate the cumulative sum
        do i_daycal = 2, 13
          daycal0(i_daycal)=daycal0(i_daycal - 1) + daymo0(i_daycal -1)
        end do
